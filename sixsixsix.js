...
function getAuthTokens() {
    ...
    try {
        // Baca file, pisahkan berdasarkan baris baru, dan hapus baris kosong/spasi
        const fileContent = fs.readFileSync(TOKEN_FILE_PATH, 'utf8');
        const tokens = fileContent.split('\n')
            .map(line => line.trim())
            .filter(token => token.length > 0); // Hapus baris kosong
...
async function checkAccountInfo(client) {
    try {
        ...
        console.log(`[INFO AKUN] Akun Email: ${email} | Kredit Saat Ini: ${credits}`);
        ...
        console.error(`[ERROR INFO] Gagal mendapatkan info akun. Status: ${status}, Data: ${data}`);
        return null;
    }
}
...
async function claimDailyCredits(client, accountIdentifier) {
    ...
    try {
        ...
        console.error(`[KLAIM ERROR] Akun ${accountIdentifier}: Gagal mengklaim. Status: ${status}, Detail: ${error.message}`);
    }
}
...
async function runAccountBot(token, index) {
    ...
    const identifier = info ? info.email : accountLabel; // Gunakan email sebagai ID jika berhasil
    ...
    // Menggunakan IIFE untuk memastikan variabel identifier di-capture dengan benar
...
async function runBot() {
    ...
    // Jalankan bot untuk setiap akun secara paralel
    for (let i = 0; i < tokens.length; i++) {
        // Timeout singkat untuk mencegah rate limit pada saat start-up
